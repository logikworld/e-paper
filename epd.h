...
// AJOUT KEVIN le 15/04/2020
void EPD_loadImg(unsigned char* image_buffer,unsigned long Imgsize)
{
    if (DEBUGKL) Serial.print("\r\n EPD_loadPict , taille:");
    int index = 0;
    //String p = image_buffer;

    // Get the length of the image data begin
    int DataLength = Imgsize;
    if (DEBUGKL) Serial.println(DataLength);
    if (DEBUGKL) Serial.println("0---------50--------%");
    // Enumerate all of image data bytes
    int pourcentage=DataLength/20;
    int cptpourc=0;
    // Enumerate all of image data bytes
    while (index < DataLength)
    {
        // Get current byte from obtained image data
        int valueA = image_buffer[index];
        cptpourc=cptpourc+1;
        if (cptpourc>=pourcentage)
        {
          cptpourc=0;
          if (DEBUGKL) Serial.print("*");
        }
        ...
        EPD_SendData((byte)image_buffer[index]);

        // Increment the current byte index on 2 characters
        index += 1;
    }
    if (DEBUGKL) Serial.println("");
}

//FIN AJOUT KEVIN 15/04/2020
